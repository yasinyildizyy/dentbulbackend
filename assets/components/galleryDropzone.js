...
function GalleryDropzone(props) {
  ...
  const postUrl = () => {
    return `/_api/admin/galleries?galleryCategoryId=${props.galleryCategoryId}`;
  }

  const getFiles = async () => {
    const response = await network.getHydraMembers(
      true,
      `/_api/admin/galleries`,
      {
        'category': props.galleryCategoryId,
        'pagination': false
      }
    );
    setFiles(response);
  ...
  const updateFilesSort = async () => {
    files.map((file, index) => {
      network.put(`/_api/admin/galleries/${file.id}`, {
        'position': index
      }).then();
    })
  ...
  const deleteFile = async (id) => {
    const response = await network.delete(`/_api/admin/galleries/${id}`);
